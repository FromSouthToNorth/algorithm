 ...
 *
 * <p>现在从源方格source = [s<sub>x</sub>, s<sub>y</sub>]开始出发，意图赶往目标方格target = [t<sub>x</sub>, t<sub>y</sub>] 。数组 blocked 是封锁的方格列表，其中每个 blocked[i] = [x<sub>i</sub>, y<sub>i</sub>] 表示坐标为 (xi, yi) 的方格是禁止通行的。</p>
 *
